...
namespace JsonConverterGenerator
{
    ...
    public class JsonConverterForJsonConverterGeneratorBasicPersonWithComplexTypes: JsonConverter<BasicPersonWithComplexTypes>
    {
        private static ReadOnlySpan<byte> AgeBytes => new byte[3] { (byte)'A', (byte)'g', (byte)'e' };
        private static ReadOnlySpan<byte> FirstBytes => new byte[5] { (byte)'F', (byte)'i', (byte)'r', (byte)'s', (byte)'t' };
        private static ReadOnlySpan<byte> HomeAddressBytes => new byte[11] { (byte)'H', (byte)'o', (byte)'m', (byte)'e', (byte)'A', (byte)'d', (byte)'d', (byte)'r', (byte)'e', (byte)'s', (byte)'s' };
        private static ReadOnlySpan<byte> OtherAddressesBytes => new byte[14] { (byte)'O', (byte)'t', (byte)'h', (byte)'e', (byte)'r', (byte)'A', (byte)'d', (byte)'d', (byte)'r', (byte)'e', (byte)'s', (byte)'s', (byte)'e', (byte)'s' };
        
        private bool _checkedForBasicJsonAddressConverter;
        private JsonConverter<BasicJsonAddress> _basicJsonAddressConverter;
        private JsonConverter<BasicJsonAddress> GetBasicJsonAddressConverter(JsonSerializerOptions options)
        {
            if (!_checkedForBasicJsonAddressConverter && _basicJsonAddressConverter == null && options != null)
            {
                _basicJsonAddressConverter = (JsonConverter<BasicJsonAddress>)options.GetConverter(typeof(BasicJsonAddress));
                _checkedForBasicJsonAddressConverter = true;
            }
            
            return _basicJsonAddressConverter;
        }
        
        private bool _checkedForListBasicJsonAddressConverter;
        private JsonConverter<List<BasicJsonAddress>> _listBasicJsonAddressConverter;
        private JsonConverter<List<BasicJsonAddress>> GetListBasicJsonAddressConverter(JsonSerializerOptions options)
        {
            if (!_checkedForListBasicJsonAddressConverter && _listBasicJsonAddressConverter == null && options != null)
            {
                _listBasicJsonAddressConverter = (JsonConverter<List<BasicJsonAddress>>)options.GetConverter(typeof(List<BasicJsonAddress>));
                _checkedForListBasicJsonAddressConverter = true;
            }
            
            return _listBasicJsonAddressConverter;
        }
        
        public override BasicPersonWithComplexTypes Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            // Validate that the reader's cursor is at a start token.
            if (reader.TokenType != JsonTokenType.StartObject)
            {
            ...
            // Create returned object. This assumes type has public parameterless ctor.
            JsonConverterGenerator.BasicPersonWithComplexTypes value = new JsonConverterGenerator.BasicPersonWithComplexTypes();
            ...
            while (true)
            {
                ...
                // Determine if JSON property matches 'HomeAddress'.
                else if (HomeAddressBytes.SequenceEqual(propertyName))
                {
                    JsonConverter<BasicJsonAddress> converter = GetBasicJsonAddressConverter(options);
                    if (converter != null)
                    {
                        value.HomeAddress = converter.Read(ref reader, typeToConvert, options);
                    }
                    else
                    {
                        value.HomeAddress = JsonSerializer.Deserialize<BasicJsonAddress>(ref reader, options);
                    }
                }
                // Determine if JSON property matches 'OtherAddresses'.
                else if (OtherAddressesBytes.SequenceEqual(propertyName))
                {
                    JsonConverter<List<BasicJsonAddress>> converter = GetListBasicJsonAddressConverter(options);
                    if (converter != null)
                    {
                        value.OtherAddresses = converter.Read(ref reader, typeToConvert, options);
                    }
                    else
                    {
                        value.OtherAddresses = JsonSerializer.Deserialize<List<BasicJsonAddress>>(ref reader, options);
                    }
                }
            }
        ...
        public override void Write(Utf8JsonWriter writer, BasicPersonWithComplexTypes value, JsonSerializerOptions options)
        {
            if (value == null)
            {
                writer.WriteNullValue();
            ...
            writer.WritePropertyName(HomeAddressBytes);
            {
                JsonConverter<BasicJsonAddress> converter = GetBasicJsonAddressConverter(options);
                if (converter != null)
                {
                    converter.Write(writer, value.HomeAddress, options);
                }
                else
                {
                    JsonSerializer.Serialize<BasicJsonAddress>(writer, value.HomeAddress, options);
                }
            }
            
            writer.WritePropertyName(OtherAddressesBytes);
            {
                JsonConverter<List<BasicJsonAddress>> converter = GetListBasicJsonAddressConverter(options);
                if (converter != null)
                {
                    converter.Write(writer, value.OtherAddresses, options);
                }
                else
                {
                    JsonSerializer.Serialize<List<BasicJsonAddress>>(writer, value.OtherAddresses, options);
                }
            }
